...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            ...
            # a <video> is treated as `num_video_images * <image>`
            ...
            # <video><video><image><image>\nxxxxxxxxxxxxx -> `num_video_images*<image>``num_video_images*<image>`<image><image>\nxxxxxxxxxxxxx
            # <video>\nxxxxxxxxxxxxx -> `num_video_images*<image>`\nxxxxxxxxxxxxx
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            disaster_map = {
                ...
                "hurricane-matthew": 3,
                "midwest-flooding": 4,
                "santa-rosa-wildfire": 5,
                "guatemala-volcano": 6,
                "palu-tsunami": 7,
                "hurricane-florence": 8,
                "mexico-earthquake": 9,
            }
            ...
            path_field = self.list_data_dict[i].get("video", self.list_data_dict[i].get("image", [""]))[0]
            parts = path_field.split('/')
            if len(parts) >= 5:
                disaster_type = parts[4]
            else:
                disaster_type = "hurricane-harvey"
            disaster_id = disaster_map.get(disaster_type, 0)
            data_dict["disaster_id"] = torch.tensor(disaster_id, dtype=torch.long)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            image_data = instance.get("image", [dummy_image])
...
# ==================== Disaster ID Handling ======================
        disaster_ids = []
        for instance in instances:
            if "disaster_id" in instance:
                disaster_ids.append(instance["disaster_id"])
            else:
# fallback to default ID = 0
                disaster_ids.append(torch.tensor(0, dtype=torch.long))

            batch["disaster_ids"] = torch.stack(disaster_ids)
