...
class LanguageBindImageTower(nn.Module):
    ...
    def forward(self, images):
        ...
        if type(images) is list:
                for image in images:
            # 输出所有隐藏层特征（而非仅最后一层）
                    forward_outs = self.image_tower(
                        image.unsqueeze(0), 
                        output_hidden_states=True  # 获取所有层特征
                    )
            # 收集多尺度特征（如第4、8、12层）
                multi_scale_feats = [
                    self.feature_select(forward_outs.hidden_states[i]) 
                    for i in [4, 8, 12]
                ]
                image_features.append(torch.stack(multi_scale_feats))
        else:
            forward_outs = self.image_tower(images, output_hidden_states=True)
            multi_scale_feats = [
                self.feature_select(forward_outs.hidden_states[i]) 
                for i in [4, 8, 12]
            ]
            image_features = torch.stack(multi_scale_feats)
        return image_features  # 形状：[num_skip_layers, B, T, C]
