...
class ResNetFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        x = self.transform(x) # Apply the resize and to tensor
...
class CLIPVisionTransformerWithResNet(nn.Module):
    def __init__(self, config: CLIPVisionConfig, resnet_type='resnet50'):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
        ...
        # ResNet特征提取器
        self.resnet_extractor = ResNetFeatureExtractor(resnet_type=resnet_type)
    ...
    def forward(self, pixel_values: Optional[torch.FloatTensor] = None, output_attentions: Optional[bool] = None,
                output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None):
        if pixel_values is None:
            raise ValueError("You have to specify pixel_values")

        # 使用 ResNet 提取局部特征
        resnet_features = self.resnet_extractor(pixel_values)

        # 继续进行 ViT 特征提取
        hidden_states = self.embeddings(pixel_values)
        hidden_states = self.patch_dropout(hidden_states, B, T)  # 对 Patch Dropout 进行处理
        hidden_states = self.pre_layrnorm(hidden_states)

        # ViT encoder 进行全局上下文特征提取
        encoder_outputs = self.encoder(
            inputs_embeds=hidden_states,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        ...
        # 将 ResNet 特征和 ViT 特征融合
        last_hidden_state = encoder_outputs[0]
        pooled_output = last_hidden_state[:, 0, :]
        pooled_output = self.post_layernorm(pooled_output)

        # 对 ResNet 特征进行池化（如果需要）
        resnet_pooled_output = resnet_features.mean(dim=[2, 3])  # 使用池化层进行降维
        combined_output = torch.cat((pooled_output, resnet_pooled_output), dim=-1)

        if not return_dict:
            return (last_hidden_state, combined_output) + encoder_outputs[1:]

        return BaseModelOutputWithPooling(
            last_hidden_state=last_hidden_state,
            pooler_output=combined_output,
            hidden_states=encoder_outputs.hidden_states,
            attentions=encoder_outputs.attentions,
        )
...
#class CLIPVisionModel(CLIPPreTrainedModel):
    ...
    #) -> Union[Tuple, BaseModelOutputWithPooling]:
        ...
        #return self.vision_model(
            ...
            #return_dict=return_dict,
        #)
class CLIPModelWithResNet(CLIPPreTrainedModel):
    def __init__(self, config: LanguageBindImageConfig):
        super().__init__(config)

        # 视觉模型：包含ResNet和ViT融合
        self.vision_model = CLIPVisionTransformerWithResNet(config.vision_config)
        # 文本模型
        self.text_model = CLIPTextTransformer(config.text_config)

        self.visual_projection = nn.Linear(config.vision_embed_dim, config.projection_dim, bias=False)
        self.text_projection = nn.Linear(config.text_embed_dim, config.projection_dim, bias=False)
        self.logit_scale = nn.Parameter(torch.tensor(config.logit_scale_init_value))

        self.post_init()

    def forward(self, input_ids=None, pixel_values=None, attention_mask=None, position_ids=None, return_loss=False):
        vision_outputs = self.vision_model(pixel_values=pixel_values)
        text_outputs = self.text_model(input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids)

        image_embeds = vision_outputs[1]  # 视觉特征
        image_embeds = self.visual_projection(image_embeds)
        text_embeds = text_outputs[1]  # 文本特征
...
class LanguageBindImage(CLIPPreTrainedModel):
    ...
    def __init__(self, config: LanguageBindImageConfig):
        ...
        self.vision_model = CLIPVisionTransformerWithResNet(vision_config)
