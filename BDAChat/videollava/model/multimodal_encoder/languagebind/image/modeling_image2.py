...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: LanguageBindImageConfig):
        ...
        # 修改CNN输入通道处理
        self.cnn_model = MultiScaleCNN(in_channels=3, out_channels=config.hidden_size)
        self.channel_adjust = nn.Conv2d(config.hidden_size, 3, kernel_size=1) if config.hidden_size != 3 else None
    ...
    def forward(
        ...
        # 调试输入形状
        print(f"Input shape to encoder layer: {hidden_states.shape}")  # 调试用
        
        # 处理CNN输入
        if hidden_states.dim() == 3:
            # 假设输入是 [batch, seq_len, embed_dim]
            # 转换为图像格式 [batch, channels, height, width]
            seq_len = hidden_states.shape[1]
            h = w = int(math.sqrt(seq_len))
            if h * w != seq_len:
                raise ValueError(f"Sequence length {seq_len} is not perfect square")
            
            cnn_input = hidden_states.permute(0, 2, 1).view(hidden_states.shape[0], -1, h, w)
            
            # 通道调整
            if self.channel_adjust is not None:
                cnn_input = self.channel_adjust(cnn_input)
            elif cnn_input.shape[1] != 3:
                cnn_input = cnn_input[:, :3]  # 只取前3个通道
            
            print(f"CNN input shape: {cnn_input.shape}")  # 调试用
            cnn_features = self.cnn_model(cnn_input)
        else:
            raise ValueError(f"Unexpected input shape: {hidden_states.shape}")

        # 处理CNN输出
        hidden_states = rearrange(cnn_features, 'b c h w -> b (h w) c')
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, MultiScaleCNN):
            # Initialize CNN weights similarly to CLIP vision embeddings
            nn.init.normal_(module.conv1.weight, mean=0.0, std=factor * 0.02)
            nn.init.normal_(module.conv2.weight, mean=0.0, std=factor * 0.02)
            nn.init.normal_(module.conv3.weight, mean=0.0, std=factor * 0.02)
            nn.init.normal_(module.conv4.weight, mean=0.0, std=factor * 0.02)
            nn.init.normal_(module.final_conv.weight, mean=0.0, std=factor * 0.02)
...
class MultiScaleCNN(nn.Module):
    def __init__(self, in_channels=16, out_channels=64):  # 修改输入通道为16
        super().__init__()
        ...
        # 第四层卷积（下采样）
        self.conv4 = nn.Conv2d(out_channels*4, out_channels*8, kernel_size=3, stride=2, padding=1)
        self.conv4_bn = nn.BatchNorm2d(out_channels*8)
        
        # 上采样层
        self.upconv4 = nn.ConvTranspose2d(out_channels*8, out_channels*4, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x_up4 = self.upconv4(x4)
        x_up3 = self.upconv3(x_up4 + x3)
        x_up2 = self.upconv2(x_up3 + x2)
        
        out = self.final_conv(x_up2 + x1)
        return out
class CLIPVisionTransformer(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.cnn_backbone = MultiScaleCNN(in_channels=3, out_channels=64)
    ...
    def forward(
        ...
        cnn_features = cnn_features.flatten(1)  # Flatten CNN output
        vision_features = encoder_outputs[0]  # ViT output
        combined_features = torch.cat([vision_features, cnn_features], dim=1)  # Concatenate along feature dimensio
        pooled_output = combined_features[:, 0, :]
        #last_hidden_state = encoder_outputs[0]
...
class LanguageBindImage(CLIPPreTrainedModel):
    ...
    def __init__(self, config: LanguageBindImageConfig):
        ...
        self.cnn_model = MultiScaleCNN(in_channels=3, out_channels=64)
        #self.visual_projection = nn.Linear(self.vision_embed_dim, self.projection_dim, bias=False)
        self.visual_projection = nn.Linear(self.vision_embed_dim + 64, self.projection_dim, bias=False)  # 融合CNN和ViT的特征
    ...
    def get_image_features(self, pixel_values: Optional[torch.FloatTensor] = None,
                           output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None,
                           return_dict: Optional[bool] = None) -> torch.FloatTensor:

        # 获取ViT特征
        vision_outputs = self.vision_model(pixel_values=pixel_values, output_attentions=output_attentions,
                                           output_hidden_states=output_hidden_states, return_dict=return_dict)
        pooled_output_vit = vision_outputs[1]  # 获取图像的pooled输出（ViT）

        # 获取CNN特征（多尺度）
        pooled_output_cnn = self.cnn_model(pixel_values)  # 通过CNN模型提取多尺度特征

        # 合并ViT和CNN特征
        combined_features = torch.cat([pooled_output_vit, pooled_output_cnn.flatten(1)], dim=1)  # 拼接ViT和CNN特征

        image_features = self.visual_projection(combined_features)
        return image_features
    ...
    def forward(
        ...
        image_embeds_vit = vision_outputs[1]  # ViT embeddings
        image_embeds_cnn = vision_outputs[0]  # CNN features (multi-scale)

        # Combine CNN and ViT features (concatenation or addition)
        combined_image_embeds = torch.cat([image_embeds_vit, image_embeds_cnn.flatten(1)],
                                          dim=1)  # Concatenate along feature dimension
        #image_embeds = vision_outputs[1]
        #image_embeds = self.visual_projection(image_embeds)
        image_embeds = self.visual_projection(combined_image_embeds)
