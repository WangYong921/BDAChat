...
class HybridVisionTransformer(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        # CNN-based feature extraction (used for capturing local, fine-grained information)
        self.cnn = nn.Sequential(
            nn.Conv2d(config.num_channels, 64, kernel_size=3, stride=1, padding=1),  # Example CNN block
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(64, 3, kernel_size=3, stride=1, padding=1),  # Ensure the output has 3 channels for RGB
            nn.ReLU(),
        )
    ...
    def forward(
        ...
        # Apply CNN to extract local features
        cnn_features = self.cnn(pixel_values)

        # Now, apply the Vision Transformer to capture the spatial information
        hidden_states = self.embeddings(cnn_features)

        hidden_states = self.patch_dropout(hidden_states, pixel_values.shape[0], 1)  # Assuming T=1 for simplicity
...
class CLIPVisionModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig):
        super().__init__(config)
        self.vision_model = HybridVisionTransformer(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class LanguageBindImage(CLIPPreTrainedModel):
    ...
    def __init__(self, config: LanguageBindImageConfig):
        ...
        self.vision_model = HybridVisionTransformer(vision_config)
