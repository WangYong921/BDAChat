...
parser.add_argument('--viz_dir', type=str, default='./viz_road/',
...
def evaluate_eng(img_dir, gt_dir, viz_dir, model, img_size=768):
    val_list = [f for f in os.listdir(img_dir) if f.endswith('_post_disaster.png')]
    ...
    for i, name in tqdm(enumerate(val_list), total=len(val_list), desc='Evaluation'):
        ...
        mask_name = name.replace('_post_disaster', '_pre_disaster')
        ...
        vis_mask = 255 * mask
        _mask_rgb = np.stack([vis_mask] * 3, axis=2)  
...
def main(args):
    ...
    if args.name == 'b_adapter_sam_sp24':
        ...
        model.load_state_dict(torch.load(r'\path\to\b_adapter_sam_60.th'),
                              strict=False)
        resize_model_pos_embed(model, args.image_size)  
        ...
        model, encoder_global_attn_indexes = build_sam_vit_b_adapter_linknet_multi_lora(
            args.SAM_pretrained_path, image_size=768)  
        model = torch.nn.DataParallel(model, device_ids=range(torch.cuda.device_count()))
        model.load_state_dict(
            torch.load(r'\path\to\b_adapter_sam_multi_lora32_60.th'),
            strict=False)
        resize_model_pos_embed(model, args.image_size)  
        ...
        model = torch.nn.DataParallel(model, device_ids=range(torch.cuda.device_count()))
        model.load_state_dict(
            torch.load(r'\path\to\b_adapter_sam_lora96_96_60.th'),
            strict=False)
        resize_model_pos_embed(model, args.image_size)  
