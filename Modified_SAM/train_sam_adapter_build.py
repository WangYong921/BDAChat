...
parser.add_argument('--data_dir', type=str, default='/mnt/3.8-2/xView2_baseline/train_images_labels_targets/xBD/guatemala-volcano/',
...
parser.add_argument('--viz_dir', type=str, default='./record_bd/viz/',
                    help='Path to save weight path')
parser.add_argument('--val_img_dir', type=str, default='/mnt/3.8-2/xView2_baseline/train_images_labels_targets/xBD/guatemala-volcano/images/',
                    help='Path to ')
parser.add_argument('--val_gt_dir', type=str, default='/mnt/3.8-2/xView2_baseline/train_images_labels_targets/xBD/guatemala-volcano/masks/',
                    help='Path to ')
...
def evaluate(img_dir, gt_dir, viz_dir, model, encoder_global_attn_indexes, img_size=1024):
    ...
    for i, name in tqdm(enumerate(val), total=len(val), desc='Evaluation'):
        ...
        if 'paris' or 'khart' in viz_dir:
            ...
            if img.shape[0] > 672 or img.shape[1] > 672:
                img = cv2.resize(img, (672, 672), interpolation=cv2.INTER_AREA)
                m, n = 672, 672

            tem = np.zeros((672, 672, 3), dtype=img.dtype)
            tem[:m, :n, :] = img
...
# === 仅保留包含 `_pre_disaster` 且 mask 存在的图像 ID ===
    all_ids = [
        fname[:-4] for fname in os.listdir(image_dir)
        if fname.endswith(".png") and "_pre_disaster" in fname
    ]

    train_list = []
    for id in all_ids:
        image_path = os.path.join(image_dir, f"{id}.png")
        mask_path = os.path.join(mask_dir, f"{id}.png")
        if os.path.exists(mask_path):
            train_list.append(id)
        else:
            print(f"[SKIP] No mask found for: {id}")

    print(f"[DEBUG] Total usable pre-disaster samples: {len(train_list)}")

    dataset_train = SP2Build_ImageFolder(train_list, args.data_dir, size=(1024, 1024))
    # print(dataset_train)
    print(f"[DEBUG] Found {len(train_list)} samples from images/")
...
# Debug 输出张量状态
            #print("[DEBUG] y_true:", y_true.shape, y_true.dtype, y_true.min().item(), y_true.max().item())
            #print("[DEBUG] y_pred:", y_pred.shape, y_pred.dtype)
...
# 正式计算 loss
        ...
        if epoch % 10 == 0:
            ...
            khart_img_dir = '/mnt/3.8-2/xView2_baseline/train_images_labels_targets/xBD/guatemala-volcano/images/'
            khart_gt_dir = '/mnt/3.8-2/xView2_baseline/train_images_labels_targets/xBD/guatemala-volcano/masks/'
            #tb = evaluate(
    ...
    save(model_without_ddp, save_path + '/' + args.name + '_150.th')
